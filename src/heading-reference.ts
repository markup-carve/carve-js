import type { Document, Heading, InlineNode, Link, Text } from './ast.js'
import { inlineText } from './heading-ids.js'
import type { CarveExtension, InlineMatch } from './extension.js'
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (node.type === 'link' && node.attrs?.keyValues?.[MARK] !== undefined) {
      ...
      if (id !== undefined && counts.get(norm) === 1) {
        ...
        if (link.attrs.order) link.attrs.order = link.attrs.order.filter((s) => s !== MARK)
      } else {
        // Missing or duplicate heading: fall back to literal source text.
        const literal =
          target === display ? `[[${target}]]` : `[[${target}|${display}]]`
        nodes[i] = { type: 'text', value: literal } as Text
      }
      continue
    }
    const kids = (node as { children?: InlineNode[]; content?: InlineNode[] }).children ??
      (node as { content?: InlineNode[] }).content
    if (Array.isArray(kids)) resolveInlines(kids, targets, counts)
  }
}
 ...
 * ```
 */
export function headingReference(opts: HeadingReferenceOptions = {}): CarveExtension {
  const classes = (opts.cssClass ?? 'heading-ref').split(' ').filter((c) => c !== '')

  return {
    name: 'heading-reference',
    matchInline(text, pos): InlineMatch | null {
      ...
      if (first === undefined || first === ']' || first === '|' || first === '#') return null
    ...
    beforeRender(doc: Document): Document {
      ...
      for (const node of doc.children) {
        ...
        if (!targets.has(text)) targets.set(text, id)
      }
      // Resolve references anywhere in the document (headings, paragraphs,
      // and nested block containers).
      for (const node of doc.children) walkBlock(node, targets, counts)
      return doc
    },
  }
}
...
function walkBlock(
  ...
  switch (node.type) {
    ...
    case 'admonition':
      ...
      break
    case 'list':
      for (const it of node.items) it.children.forEach((c) => walkBlock(c, targets, counts))
      break
    case 'table':
    ...
    case 'definition_list':
      for (const it of node.items) {
        for (const t of it.terms) resolveInlines(t, targets, counts)
        for (const d of it.definitions) for (const b of d) walkBlock(b, targets, counts)
      }
      break
    default:
      break
