...
function stripHref(a: Attrs): Attrs {
  if (!a.keyValues) return a
  const hrefKeys = Object.keys(a.keyValues).filter((k) => k.toLowerCase() === 'href')
  if (hrefKeys.length === 0) return a
  const keyValues = { ...a.keyValues }
  for (const k of hrefKeys) delete keyValues[k]
...
function renderGlossary(
  ...
  if (defListsOf(node).length === 0) return undefined as unknown as string
  ...
  for (const child of node.children) {
    ...
    const rows: string[] = []
    for (const item of (child as DefinitionList).items) {
      ...
      for (const def of item.definitions) rows.push(`${inner}<dd>${renderDef(def, ctx)}</dd>`)
    }
...
function renderDef(def: BlockNode[], ctx: BlockExtensionRenderContext): string {
  if (def.length === 1 && def[0]!.type === 'paragraph')
    return ctx.renderInlines((def[0] as Paragraph).children)
