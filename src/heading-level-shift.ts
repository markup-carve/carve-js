import type { BlockNode, Document, HeadingLevel } from './ast.js'
...
function shiftBlock(node: BlockNode, shift: number): void {
  switch (node.type) {
    case 'heading':
      // Cap at h6 (matches carve-php's Heading::setLevel clamp).
      node.level = Math.min(6, node.level + shift) as HeadingLevel
      break
    case 'blockquote':
    case 'div':
    case 'admonition':
      ...
      break
    case 'list':
      for (const it of node.items) it.children.forEach((c) => shiftBlock(c, shift))
      break
    case 'definition-list':
      for (const it of node.items)
        for (const d of it.definitions) for (const b of d) shiftBlock(b, shift)
      break
    case 'figure':
      if (node.target.type === 'blockquote') shiftBlock(node.target, shift)
      break
    default:
      break
  }
 ...
 * ```
 */
export function headingLevelShift(opts: HeadingLevelShiftOptions = {}): CarveExtension {
