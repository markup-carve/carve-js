...
function defaultSlug(page: string): string {
  return page
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '-')
 ...
 * ```
 */
export function wikilinks(opts: WikilinksOptions = {}): CarveExtension {
  ...
  return {
    ...
    matchInline(text, pos) {
      ...
      if (rawPage.includes(']')) return null
      let display: string | null = bar >= 0 ? body.slice(bar + 1).trim() : null
      ...
      if (display === null) display = page || anchor

      const classes = cssClass.split(' ').filter((c) => c !== '')
      const keyValues: Record<string, string> = { 'data-wikilink': page }
      if (opts.newWindow) {
        keyValues.target = '_blank'
        keyValues.rel = 'noopener'
      }
      const attrs: Attrs = { classes, keyValues }
