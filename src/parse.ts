...
// found by balance (matchBracket), so the inner text may hold nested
...
// inside a quoted value, so the close `}` is the first one outside quotes
// (djot "don't mind braces in quotes"). RE_SPAN_TAIL's body is `+` (an empty
