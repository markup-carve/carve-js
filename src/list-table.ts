 ...
 */
export function listTable(): CarveExtension {
  return {
    name: 'list-table',
    blockRenderers: {
      admonition: (node, ctx) => {
        ...
        if (adm.kind !== 'list-table') return undefined
...
/** One cell, plus any stray (non-list) blocks the parser left after the inner
 *  cell list - they belong to the most recently opened cell. Recorded here
 *  WITHOUT mutating the AST so the defer decision is made on a pristine tree. */
interface CellEntry {
  cell: ListItem
  extras: BlockNode[]
}

/** A grid entry, positionally indexed by source cell column. Mirrors the
 *  carve-js pipe-table grid (render-html `renderTable`): a `^`/`<` marker grows
 *  the rowspan/colspan of its source and is then skipped; an unmergeable marker
 *  stays as a (rendered-empty) cell. This guarantees span markup identical to
 *  the equivalent pipe table. */
interface GridEntry {
  cell: ListItem
  extras: BlockNode[]
  /** `^` or `<` when the cell is a bare span marker, else null. */
  marker: '^' | '<' | null
  rowspan: number
  colspan: number
...
function renderListTable(node: Admonition, ctx: BlockExtensionRenderContext): string | null {
  ...
  if (node.children.length !== 1 || node.children[0]!.type !== 'list') return null
  const outerList = node.children[0] as List
  ...
  if (footGrid.length > 0) {
    ...
    footGrid.forEach((gridRow, offset) => {
      foot.push(`${pad}    ${renderRow(gridRow, footerStart + offset)}`)
    })
...
interface Placement {
  /** Output start column of each source cell, per row (skip cells: -1). */
  cols: number[][]
 ...
 */
function extractCells(rowItem: ListItem): CellEntry[] | null {
  const cells: CellEntry[] = []
  for (const child of rowItem.children) {
    if (child.type === 'list') {
      for (const cellItem of (child as List).items) {
 ...
 * line stays literal.
 */
function markerOf(entry: CellEntry): '^' | '<' | null {
  const cell = entry.cell
  if (cell.attrs) return null

  // Trailing blocks make this a multi-block cell, not a bare marker.
  if (entry.extras.length > 0) return null

  if (cell.children.length !== 1) return null

  const paragraph = cell.children[0]!
  if (paragraph.type !== 'paragraph' || (paragraph as Paragraph).attrs) return null

  const inlines = (paragraph as Paragraph).children
  if (inlines.length !== 1) return null

  const text = inlines[0]!
  if (text.type !== 'text' || text.attrs) return null
...
function renderCell(
  ...
  const blocks: BlockNode[] = [...cell.children, ...extras]

  if (blocks.length === 1 && blocks[0]!.type === 'paragraph' && !(blocks[0] as Paragraph).attrs) {
 ...
 * source order.
 */
function renderTableAttributes(node: Admonition, ctx: BlockExtensionRenderContext): string {
  ...
  const cleaned: Attrs = {}
  if (attrs.id !== undefined) cleaned.id = attrs.id
  if (classes.length > 0) cleaned.classes = classes
  if (Object.keys(keyValues).length > 0) cleaned.keyValues = keyValues
  if (order.length > 0) cleaned.order = order
