...
- AST ingest returns a typed error for `section`, block content in table cells, and labeled or numbered math until those interchange shapes are implemented.
...
- The CLI runs when launched through a symlink (#1909). `npx`, a `node_modules/.bin` shim and a global install all hand node the link rather than the file, so the entry guard's raw string comparison failed and the process exited 0 having rendered nothing; both sides are resolved now, which also covers a package directory that is itself a link.
...
- The Markdown importer fits each imported table body row to the header's column count (#1920), as GFM does: cells past the header width are dropped and a short row is padded with empty cells. This covers document-level tables and tables inside list items and quotes.
- The Markdown importer writes fences, list markers and nested quotes the way `carve fmt` does (#1921), so an imported document passes `fmt --check`. Fences take backticks one longer than any run in the body; a quoted `> ~~~` is a code block again rather than a paragraph; ordered items are numbered on from the list's first number; a bullet or delimiter change starts a new list with fmt's blank line between, so `- x` then `+ y` no longer merges into one list where GFM renders two; and a nested quote under a quote paragraph gets fmt's empty `>` line.
- The Markdown importer reads six shapes the way cmark-gfm does (#1941). A setext underline takes every line of its paragraph rather than only the line above it, at the top level and inside a quote or a list item alike, and no longer reaches across a nested item's lazy line. Indented code a quote opens with under item text becomes a quoted fence instead of paragraph text, while indented code under a quote paragraph that is still open stays in that paragraph, since it cannot interrupt one. A fence in an item writes its body at the item's content column rather than at column 0, which an indented opener lost the same way a tab-indented one did. A bare URL's hyphen run is escaped, Carve reading the URL as text where it rendered an en dash, and the opt-in `autolink()` extension decodes such an escape for both href and link text. The lazy row of a table a nested quote holds is written inside that quote, the way `carve fmt` writes it.
- The Markdown importer closes an unclosed fence where its container ends, keeps a quoted fence's empty quote lines and deeper markers as code, moves the lines an item holds when renumbering changes a marker's width, and writes a loose list with fmt's blank lines (#1929).
- The Markdown importer writes continuation lines the way `carve fmt` does: an over-indented line moves to the item's content column and anything that would open a block there is escaped; a marker between two list levels is the next item of the list at its level, per CommonMark 5.2; and a lazy line after a quote in an item takes the quote's marker at the quote's column (#1932).
...
- **Semantic table row partitions: `{header-rows=N footer-rows=N}` on a pipe table, `{align= valign=}` on a ListTable cell** (#1223).
- **Local ListTable headers: `header-row` on a row's first cell, `header` on any cell** (#1220, markup-carve/carve#1248).
- **`?` inherits a column's horizontal alignment**, so `?^`, `?~` and `?v` set only `valign` (#1218, markup-carve/carve#1408).
...
- **A container's span ends at its last placed child** (markup-carve/carve#1522, markup-carve/carve#1524, PART 12 §4). Published `pos` values move.
- **A `definition_list`'s span ends at its last placed child too** (markup-carve/carve#1530, PART 12 §4). Published `pos` values move.
- **A `css`-mode tabs or code-group panel carries its tab's name**, as `role="group"` plus an `aria-label`; an `aria`-mode panel (§13.3) and a `"static"` render take neither (#1265, markup-carve/carve#1489, PART 11 §13.2).
- **`codeGroup()` honors the `mode` option it used to accept and ignore**, and **an unknown `mode` is rejected** rather than silently falling back to `css` (#1265).
- **Three blank lines are a hard list boundary**, opening a new list at every level instead of loosening the current one (markup-carve/carve#1430, §11 N1a). The canonical writer separates two adjacent sibling lists with it.
- **The doubled run is the canonical arrow, in both families** (#1241, markup-carve/carve#1442). **BREAKING: `=>` no longer converts**, so `key => value` stays literal; `<=` keeps `≤`.
- **An empty brace pair is text, and `{--}` is an en dash** (#1242, #1243, markup-carve/carve#1447, markup-carve/carve#1450).
- **A lone vertical table-alignment marker is cell content** (#1216).
- **A table cell's alignment run is horizontal-first**, so `^<`, `v>` and `~>` stay literal cell content (#1219).
- **A table cell's marker run ends at a space** (markup-carve/carve#1259, PART 9 §5 T11). The canonical writer already pads every cell, so a formatted document needs no migration.
...
- **The single-hyphen arrows `<-`, `->` and `<->`** (#1241). They still render; prefer `<--`, `-->` and `<-->`.

### Removed

- **`escapedLeadingCaret` is gone from the parse tree** (#1259). It never crossed the wire (#735), so no published output moves.
...
- **A dropped empty description breaks the list rather than lending it to the term above** (markup-carve/carve#1636).
- **An import keeps what PART 11 §7 calls content, and a block that spells nothing keeps a list boundary** (#1400, PART 11 §10j). U+00A0, U+202F and U+3000 survive; an all-layout block reports `element-dropped`.
- **A recognized block-math div costs the same import depth as an ordinary one** (markup-carve/carve#1514, markup-carve/carve#1518, PART 9 §18).
- **A BBCode list knows its closer and the column it occupies** (#1383, #1385, #1387).
- **A literal BBCode run re-enters the document at its own column** (#1384, #1386).
- **The Markdown importer escapes a literal backtick and an attribute brace instead of carrying them across bare** (#1389).
- **A fence info string does not lose what follows a backtick** (#1392).
- **`toAstJson` normalizes an ordered list's `start` of 1 away**, as `resources/ast-schema.json` describes (#1391, markup-carve/carve-rs#1293).
- **An element with no destination builds no link or image**: `<a href="">` and an `<img>` with no `src` come back as their content, reported `element-unwrapped` (ruling markup-carve/carve#1609, `spec/docs/html-import.md`).
- **The canonical writer and the converters' escaper escape a symbol-opening colon** (ruling markup-carve/carve#1609, markup-carve/carve-php#1610, PART 11 §2). `a : b : c` and a URL's colon are unchanged.
- **A table cell whose whole payload is a span marker is escaped**, instead of being re-read as a rowspan (ruling markup-carve/carve#1609, PART 11 §6f).
- **A footnote definition's span ends at its last placed child** (#1364, PART 12 §4). A note whose body continues past the definition still covers it (markup-carve/carve#1571).
- **The BBCode importer reads `[noparse]`**, consuming both tags and leaving the content literal (#1368).
- **A BBCode code run keeps the markup it encloses** (#1368, markup-carve/carve-php#1206).
- **Writing a container's missing closer no longer changes the list's tightness** (#1370, ruling markup-carve/carve#1602, PART 11 §1).
- **The HTML importer drops the whitespace a Carve line drops** (#1369), across a hard break, a table cell, a caption's marker separator and a lazy continuation.
- **The canonical writer keeps a heading's leading tab** (#1356, markup-carve/carve#1581, PART 11 §1).
- **An import report is ordered by the losing element's document position** (markup-carve/carve#1586).
- **A definition term's span ends at its last placed child** (#1349, PART 12 §4 and PART 2's NO TRAILING WHITESPACE clause).
- **A heading's span ends at its last placed child** (#1348, PART 12 §4 and PART 2's NO TRAILING WHITESPACE clause).
- **A footnote definition's span ends at its body, not at the blank line that ends it** (#1347, PART 12 §4).
- **A table `<caption>` is numbered among the table's child nodes**, not ranked among captions (markup-carve/carve#1560, PART 12 §16).
- **A derived endnotes section is not a reported loss** (markup-carve/carve-php#1588, ruling markup-carve/carve#1500). An authored `class`, and a non-default `aria-label`, are still reported.
- **A non-`li` child of a list keeps its content and is reported `element-unwrapped`** (#1340, markup-carve/carve-rs#1266).
- **A block below a sub-list in a tight item stays in the outer item** (markup-carve/pandoc-carve#135, PART 9 §17 L3).
- **A verbatim span touching a fence run no longer gains a trailing space**, so `toHtml(fmt(x))` matches `toHtml(x)` again (#1338, PART 11 §1).
- **A derived admonition title id is recognized when the id namespace collides** (#1336, markup-carve/carve-rs#1264).
- **An HTML import rebuilds the container the renderer wrote**, rather than a generic `div` with a class (#1316, markup-carve/carve#1502, PART 9 §16a). Imported source and the `htmlToAst` tree both change.
- **A consumed caption reports the attributes it drops** (#1332). The conversion is unchanged; the report gains the rows.
- **A diagnostic on a bare inline run is numbered among the body children** (markup-carve/carve#1554, PART 12 §16).
- **A marker at an item's content column opens a sublist, first in the item or not** (markup-carve/carve#1517, PART 9 §24 C3). A marker on a block quote's lazy continuation is still text (#1200).
- **A blank verbatim line inside a block quote drops the prefix's trailing space** (markup-carve/carve#1544, PART 11 §7).
- **A tab-bearing line block stanza publishes the position of its comment node** (#1323, markup-carve/carve-rs#1247, PART 12 §4).
- **The AST-JSON ingest replaces U+0000, as the parse boundary already does** (#1311, #1294, markup-carve/carve#1528, PART 12 §21). `bbcodeToCarve` performs the same replacement.
- **An offset past a line the block layer removed names the line it is on** (#1305, markup-carve/carve#1534, PART 12 §4). Published `pos` values move on two shapes.
- **An escape escalation reaches the block that failed, not the document** (#1298, markup-carve/carve#1516, markup-carve/carve#1546, PART 11 §2b).
- **The canonical writer spells two sibling sub-lists inside a list item**, at the item's content column and separated by the boundary (markup-carve/carve#1501, §11 N1a and §10i).
- **An HTML import no longer bakes a derived accessible name into source** (#1278, markup-carve/carve#1500, markup-carve/carve#1511, PART 9 §16a, Extensions §1.5). A name that differs from the derived one is kept.
- **A math span survives an HTML import** (#1277, PART 9 §18), so the non-HTML writers see math again.
- **The Markdown importer replaces an authored U+0000 with U+FFFD** (#1291, #1290, markup-carve/carve#678), which CommonMark 2.3 requires.
- **The BBCode importer's stash key is picked per post, not a fixed U+E001/U+E002 pair** (#1290, #1289, markup-carve/carve-rs#1210, markup-carve/carve#678). A post that leaves no private-use run free is refused with the exported `BbcodeSentinelSpaceExhaustedError`.
- **The writer's marker-column tag and the Markdown target's escape carriers are picked per document, not fixed** (#1280, #1281, markup-carve/carve-rs#1210, markup-carve/carve#678).
- **A tab control is `type="button"`, and two marked items select one tab** (#1285, markup-carve/carve#1504, Extensions §13.3 and §13.5).
- **`ParseOptions.positions` is honored** (#1263), suppressing `pos`, `footnoteDefPos`, `termSpans`, `definitionSpans` and `definitionLines`. The default is unchanged.
- **`package.json` is importable, so the installed version can be read back** (#1257). Every other subpath stays refused.
- **The index back-link says where it goes**: the k-th is named `Back to {term} {k}` and shows `↩<sup>k</sup>`, with a new `backrefLabel` option on `index()` (markup-carve/carve#1469, PART 9 §16).
- **The footnote backlink has an accessible name**, `Back to reference` plus its ordinal (markup-carve/carve#1455, PART 9 §16).
- **A table alignment run requires a literal space separator** (#1213), so tabs remain visible cell content.
...
- **A content-column heading leaves no paragraph open** (#1210, markup-carve/carve#1377, markup-carve/carve#1392).
...
- **Definitions collected at a list item's content column close its paragraph** (markup-carve/carve#1376).
