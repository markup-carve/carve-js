...
- **An element with no destination builds no link or image** (ruling markup-carve/carve#1609, `spec/docs/html-import.md`). Carve has no spelling for an empty destination slot, so `<a href="">` and an `<img>` with no `src` were imported as `[t]()` and `![t]()` - punctuation the HTML never held, in the middle of the prose. The content stands in the element's place, an image's being its alt text, reported `element-unwrapped`; the destination is not rebuilt from a `title`, from the anchor text or from provenance, because PART 9 §25 blanks a dangerous one the same way and any such route would reconstruct it. A `title` beside an empty destination now survives as an attribute instead of vanishing as a consumed one.
