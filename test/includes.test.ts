...
describe('include rules', () => {
  ...
  describe('formatting preserves directives (spec I1)', () => {
    ...
    it('leaves a run that is not shape-well-formed to the ordinary writer', () => {
      // No closing "}}" and an empty path token: neither is a directive at
      // all, so neither is preserved, and both take the ordinary escaping
      // path. That path is now MINIMAL - the writer re-parses what it wrote
      // and escapes only where the round trip needs it - so a brace run that
      // reads back as the same text is emitted bare. What matters here is that
      // it is not PRESERVED as a directive, which the round trip below pins.
      expect(fmt('{{ oops')).toBe('{{ oops\n')
      expect(fmt('{{ }}')).toBe('{{ }}\n')
      expect(fmt(fmt('{{ oops'))).toBe(fmt('{{ oops'))
      expect(fmt(fmt('{{ }}'))).toBe(fmt('{{ }}'))
    })
...
describe('expandIncludes resource bounds', () => {
  const counting = (files: Record<string, string>) => {
    const calls: string[] = []
    const resolve = (path: string) => {
      calls.push(path)
      const source = files[path]
      return source === undefined ? null : { source, id: path }
    }
    return { calls, resolve }
  }

  const run = (source: string, files: Record<string, string>, options: Omit<IncludeOptions, 'resolve'> = {}) => {
    const { calls, resolve: r } = counting(files)
    const doc = parse(source, { positions: true })
    return { calls, result: expandIncludes(doc, source, { ...options, resolve: r }) }
  }

  it('stops resolving once the byte budget is spent', () => {
    // `b` is one byte and would fit the remaining budget, but a spent total is
    // terminal: the refusal is already decided, and resolving to reach it is
    // the amplification the budget exists to prevent (spec section 19).
    const { calls, result } = run('{{ a }}\n\n{{ b }}\n\n{{ b }}\n', { a: '1234567890', b: 'x' }, { maxBytes: 4 })
    expect(calls).toEqual(['a'])
    expect(result.warnings.map((w) => w.rule)).toEqual(['include-budget', 'include-budget', 'include-budget'])
  })

  it('refuses directives past the resolver-call bound without resolving them', () => {
    ...
    expect(result.warnings.map((w) => w.rule)).toEqual(['include-call-limit', 'include-call-limit'])
    expect(result.warnings[0]!.message).toContain('resolver call limit')
  })

  it('counts nested resolves against the same bound', () => {
    const { calls, result } = run('{{ a }}\n', { a: '{{ b }}', b: '{{ c }}', c: 'Deep.' }, { maxResolverCalls: 2 })
    expect(calls).toEqual(['a', 'b'])
    expect(renderHtml(resolve(result.doc))).not.toContain('Deep.')
  })

  it('reports a skipped directive as an unresolved dependency', () => {
    const { result } = run('{{ a }}\n\n{{ b }}\n', { a: 'One.', b: 'Two.' }, { maxResolverCalls: 1 })
    expect(result.dependencies).toEqual([
      { id: 'a', resolved: true },
      { id: 'b', resolved: false },
    ])
  ...
  it('caps retained warnings but keeps one per rule and counts the rest', () => {
    const source = Array.from({ length: 40 }, () => '{{ missing }}').join('\n\n')
    const { result } = run(source, {}, { maxWarnings: 5 })
    expect(result.warnings).toHaveLength(5)
    expect(result.suppressedWarnings).toBe(35)
    expect(new Set(result.warnings.map((w) => w.rule))).toEqual(new Set(['include-unresolved']))
  })

  it('keeps a distinct rule even after the cap is reached', () => {
    // One unresolved directive fills a cap of 1; the cycle that follows is a
    // rule not yet shown, so it survives rather than being counted away.
    const source = '{{ missing }}\n\n{{ a }}\n'
    const { result } = run(source, { a: '{{ a }}' }, { maxWarnings: 1 })
    expect(result.warnings.map((w) => w.rule)).toEqual(['include-unresolved', 'include-cycle'])
  })

  it('refuses a filesystem target over the size cap', () => {
    const root = mkdtempSync(join(tmpdir(), 'carve-cap-'))
    try {
