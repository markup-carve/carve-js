 ...
 * a marker. Canonical cell padding now separates every prefix from its content,
 * covering this case without a special-purpose guard.
 *
...
describe('fmt escapes only where the channel actually opens', () => {
  ...
  it('pads an unattributed prefixed cell on both sides', () => {
    // After a bare `=` the scan does run, and `~x~` glued to it reads as center
    // alignment with the text `x~`; canonical padding keeps it as content.
