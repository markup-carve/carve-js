 ...
 * so it is not a slice of the source the link was written at. The clone carried
 ...
 */
describe('a resolved crossref link', () => {
  ...
  it('still carries the heading text', () => {
    const link = linkOf('# Getting Started\n\nSee </#getting-started>.\n')
    const text = link.children.map((c) => ('value' in c ? c.value : '')).join('')
