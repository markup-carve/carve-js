 ...
 *
 * `style` is settled - markup-carve/carve#2267 routed it through the refusal
 * policy - and `test/a-refused-declaration-in-style-is-a-refused-attribute.test.ts`
 * owns it. The rows are read unfiltered here so a `style-unmapped` returning to
 * kept bytes fails this file too.
 */
...
describe('every element kept raw reports what is in its bytes', () => {
   ...
   * dangerous `style` added. Every one of the three is a refused attribute in the
   * kept bytes, and the `style` reads its reason off the declaration.
   */
  it('leaves no handler, denied destination or refused declaration unreported in the #2267 payload', () => {
  ...
  it('reports two refusals on one element in the order the element spells them', () => {
    expect(report('<form onclick="a()" action="javascript:b()">t</form>').slice(0, 2)).toEqual([
      ['attribute-preserved', 'error', '/form[1]', 'Preserved event-handler attribute onclick on <form> in the raw HTML this element is kept as'],
    ...
    expect(report('<form action="javascript:b()" onclick="a()">t</form>').slice(0, 2)).toEqual([
      ['attribute-preserved', 'error', '/form[1]', 'Preserved action with a denied URL scheme on <form> in the raw HTML this element is kept as'],
      ['attribute-preserved', 'error', '/form[1]', 'Preserved event-handler attribute onclick on <form> in the raw HTML this element is kept as'],
    ])
  })
  ...
  it('covers every class of refusal a descendant can carry', () => {
    const rows = (html: string) => report(html).slice(1)
  ...
  it('leaves safe removing what roundtrip keeps', () => {
    ...
    expect(report(html, 'safe')).toEqual([
